  ...
  func getDirections(origin: CLLocationCoordinate2D!, destination: CLLocationCoordinate2D!, waypoints: Array<String>!, travelMode: TravelModes!, completionHandler: ((status: String, success: Bool) -> Void)) {
    ...
    if let originLocation = origin {
      if let destinationLocation = destination {
        var directionsURLString = baseURLDirections + "origin=\(originLocation.latitude),\(originLocation.longitude)&destination=\(destinationLocation.latitude),\(destinationLocation.longitude)" // + "origin=" + originLocation.latitude + "&destination=" + destinationLocation
